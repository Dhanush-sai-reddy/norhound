#!/usr/bin/env python3
"""Run the NorHound saved-data check with one command."""
...
def main() -> int:
    ...
    if not runner.is_file() or not fixture.is_file():
        print("Starter files are missing. Put first_run.py inside the extracted norhound-starter-kit folder, then run it again.")
        return 2
    ...
    try:
        ...
        print("The saved-data check did not finish within 30 seconds. Send this exact blocker to the contest intake address.")
        return 1

    if result.returncode:
        print("The saved-data check failed. Send the text below to the contest intake address:")
    ...
    print("NorHound starter: SUCCESS")
