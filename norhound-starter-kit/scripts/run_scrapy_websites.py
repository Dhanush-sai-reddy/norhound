...
def main() -> None:
    try:
        ...
        from norway_company_agent.scrapy_crawler import NorHoundWebsiteSpider
    ...
    metadata_path = jobdir / "norhound-input.json"
    ...
    crawler = process.create_crawler(NorHoundWebsiteSpider)
    ...
    request_latencies = getattr(crawler, "norhound_request_latency_ms", [])
    response_download_latencies = getattr(crawler, "norhound_response_download_latency_ms", [])
    failure_attempt_latencies = getattr(crawler, "norhound_failure_attempt_latency_ms", [])
    selected_orgs = {row["organisation_number"] for row in crawl_targets}
    company_completion_by_org = dict(getattr(crawler, "norhound_company_completion_ms_by_org", {}))
    ...
    request_domains = getattr(crawler, "norhound_request_domains", Counter())
    response_statuses = getattr(crawler, "norhound_response_statuses", Counter())
    error_buckets = getattr(crawler, "norhound_error_buckets", Counter())
