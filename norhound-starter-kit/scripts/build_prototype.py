...
def qualification_copy(score: dict | None, independent_score: dict | None = None) -> tuple[str, str]:
    ...
    if score.get("scorer") == "norhound_all_source_completeness_v1":
...
def build(rows: list[dict], score: dict | None, control_loop: dict | None = None, independent_score: dict | None = None, external_by_org: dict[str, list[dict]] | None = None) -> str:
    ...
    score_ui = score or {}
    if score_ui.get("scorer") == "norhound_all_source_completeness_v1":
...
<title>NorHound laboratory — Norway company intelligence</title><meta name="description" content="Verified external company signals grounded in official Norwegian company records.">
<style>
...
<header class="mast"><div class="brand"><i></i>NorHound laboratory</div><div class="snapshot">100-company showcase · 22 exact LinkedIn profiles</div></header>
...
if(SCORE.scorer==='norhound_all_source_completeness_v1')$('#core-score').textContent=`${{Number(SCORE.raw_score||0).toFixed(2)}}/100 experimental · ${{Number(SCORE.awardable_score||0).toFixed(2)}}/100 strict`;
</script></body></html>'''
