...
class OperationalTelemetryMiddleware:
    def __init__(self, crawler):
        ...
        crawler.norhound_request_latency_ms = []
        crawler.norhound_response_download_latency_ms = []
        crawler.norhound_failure_attempt_latency_ms = []
        crawler.norhound_request_domains = Counter()
        crawler.norhound_response_statuses = Counter()
        crawler.norhound_error_buckets = Counter()
    ...
    def process_request(self, request, spider):
        request.meta["_norhound_started_at"] = time.monotonic()

    def _record(self, request):
        started = request.meta.get("_norhound_started_at")
        if started is not None:
            self.crawler.norhound_request_latency_ms.append((time.monotonic() - started) * 1000)
        host = (urllib.parse.urlparse(request.url).hostname or "unknown").casefold()
        self.crawler.norhound_request_domains[host] += 1

    def process_response(self, request, response, spider):
        self._record(request)
        download_latency = request.meta.get("download_latency")
        if download_latency is not None:
            self.crawler.norhound_response_download_latency_ms.append(float(download_latency) * 1000)
        self.crawler.norhound_response_statuses[str(response.status)] += 1
        return response
    ...
    def process_exception(self, request, exception, spider):
        ...
        self._record(request)
        if started is not None:
            self.crawler.norhound_failure_attempt_latency_ms.append((time.monotonic() - started) * 1000)
        self.crawler.norhound_error_buckets[type(exception).__name__] += 1


class NorHoundWebsiteSpider(scrapy.Spider):
    name = "norhound_websites"
    custom_settings = {
    ...
    def _record_company_completion(self, request):
        organisation_number = request.meta["organisation_number"]
        values = getattr(self.crawler, "norhound_company_completion_ms_by_org", {})
        ...
        self.crawler.norhound_company_completion_ms_by_org = values
